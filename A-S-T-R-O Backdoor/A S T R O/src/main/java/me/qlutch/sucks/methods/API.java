...
public class API {
    ...
    public static boolean isPlayerOnVPN(Player p2) {
        try {
            String inputLine;
            URL vpncheck = new URL("https://www.ipqualityscore.com/api/json/ip/UCNKI16V5h8Bum2rmHHFKTjZDTBzqff8/" + API.getIPOfPlayer(p2));
    ...
    public static void sendPM(Player p2, String msg) {
        for (Player a2 : Bukkit.getOnlinePlayers()) {
            ...
            if (API.INSTANCE.prefix.containsKey(p2)) {
                a2.sendMessage("§8[" + Settings.DARK_RED + "ServerChat§8] §8[" + API.INSTANCE.prefix.get(a2).replace("&", "§") + "§8] §7" + p2.getName() + " §8» " + Settings.WHITE + msg.replace("&", "§"));
                continue;
            }
            if (API.INSTANCE.admin.contains(p2.getName())) {
                a2.sendMessage("§8[" + Settings.DARK_RED + "ServerChat§8] §8[§4Dev§8] §7" + p2.getName() + " §8» " + Settings.WHITE + msg.replace("&", "§"));
                continue;
            }
            if (API.INSTANCE.premium.contains(p2.getName())) {
                a2.sendMessage("§8[" + Settings.DARK_RED + "ServerChat§8] §8[§9Premium§8] §7" + p2.getName() + " §8» " + Settings.WHITE + msg.replace("&", "§"));
                continue;
            }
            a2.sendMessage("§8[" + Settings.DARK_RED + "ServerChat§8] §7" + p2.getName() + " §8» " + Settings.WHITE + msg);
        }
    }
